...
static vector<float> matToFlat28x28(Mat s){
    ...
    vector<Point> pts; findNonZero(s,pts);
    if(pts.empty()) return vector<float>(784,0.0f);
    ...
    for(int i=0;i<28;i++) for(int j=0;j<28;j++) flat[i*28+j]=square.at<float>(i,j);
    return flat;
}
...
static bool loadDatasetFromRoot(const string& root,const vector<string>& classFolders,
    const unordered_map<string,int>& classMap,Dataset& out,int limitPerClass,bool shuffleFiles,mt19937* rng){

    out.X.clear(); out.y.clear();
    if(!fs::exists(root)||!fs::is_directory(root)){
        cerr<<"Error: dataset root not found: "<<root<<"\n";
        return false;
    }
    for(const auto& folder:classFolders){
        fs::path folderPath=fs::path(root)/folder;
        if(!fs::exists(folderPath)||!fs::is_directory(folderPath)){
            cerr<<"Error: missing folder: "<<folderPath.string()<<"\n";
            return false;
        }
        vector<fs::path> files;
        for(const auto& f:fs::directory_iterator(folderPath)){
            if(f.is_regular_file()&&isImageFile(f.path())) files.push_back(f.path());
        }
        if(files.empty()){
            cerr<<"Error: no images in folder: "<<folderPath.string()<<"\n";
            return false;
        }
        ...
        for(int i=0;i<take;i++){
            ...
            if(flat.size()==784){ out.X.push_back(std::move(flat)); out.y.push_back(classIndex); }
        }
    }
    if(out.X.empty()){
        cerr<<"Error: no samples loaded from "<<root<<"\n";
        return false;
    }
    return true;
}
...
int main(){
    ...
    const int LIMIT_TRAIN=-1;
    const int LIMIT_VAL=-1;
    const int LIMIT_TEST=-1;
    const int EPOCHS=80;
    ...
    vector<string> indexToLabel; indexToLabel.reserve(classFolders.size());
    ...
    if(!loadDatasetFromRoot(trainRoot,classFolders,classMap,train,LIMIT_TRAIN,true,&rng)) return -1;
    if(!loadDatasetFromRoot(valRoot,classFolders,classMap,val,LIMIT_VAL,false,nullptr)) return -1;
    if(!loadDatasetFromRoot(testRoot,classFolders,classMap,test,LIMIT_TEST,false,nullptr)) return -1;
    ...
    cout<<"\nPredictions (30 random TEST samples):\n";
    for(int t=0;t<30;t++){
    ...
    string exprPath="";
    for(const auto& p:exprCandidates){ if(fs::exists(p)){ exprPath=p; break; } }

    if(exprPath.empty()){
        cout<<"No expression image found in data/test_images.\n";
        ...
        if(exprBin.empty()){
            ...
            double value=0.0; string err;
            if(tryEvaluateExpression(exprPred,value,err)) cout<<"Result = "<<value<<"\n";
            else cout<<"Cannot evaluate: "<<err<<"\n";
        }
    }
